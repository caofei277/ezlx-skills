...
6. Read `manifest.json` or `flow-build.json` and improve the generated H5:
   ...
   - Replace simple text-layer PNGs with HTML text only when the font, weight, line height, color, and letter spacing can be identified reliably. Keep the raster export as a fallback during visual comparison. Treat PSD text bounds as placement coordinates, not a clipping mask: single-line text must use `white-space: nowrap` and `overflow: visible`; only explicit PSD line breaks may use preserved multiline whitespace.
