...
4. Run `scripts/export_psd.py` to generate:
   ...
   - `<PSD 文件名>.html` and `styles.css`: a responsive absolute-coordinate H5 render using the exported assets. Standalone export does not default to `index.html`; the HTML name is derived from the PSD filename.
...
5. For flow mode, model independent pages in `screens[]` and page-owned dialogs/drawers in `screens[].overlays[]`; do not ask users to set `states[].mode`. Use `transitions[].overlay` for same-page overlays and a different `to` screen ID for page navigation. Overlay controls may be written in `screens[].overlays[].elements[]`; when omitted, `build_flow.py` inherits matching IDs from the page elements based on the overlay state's outgoing transitions. It must write the resolved elements into `flow-build.json` and stop with an interaction error if any transition trigger has no clickable element with resolvable bounds. Never patch generated `app.js` to compensate for an empty overlay state. Run `scripts/analyze_fonts.py flow.json --update`, then `scripts/validate_flow.py flow.json --strict` before exporting; it automatically adds discovered PSD font mappings and rejects skeletal or placeholder flow files. Only after strict validation passes may `scripts/build_flow.py flow.json` generate the runtime. A flow that was manually reduced to `{project, screens, transitions}` is not an initialized project and must be regenerated with `init_project.py`.
   - `build_flow.py` emits one root HTML document per `screens[]` item. The filename is derived from `screens[].title` when present, otherwise `screens[].id`, sanitized for filesystem use, and made unique deterministically. Each page has a matching `<name>.assets/` directory containing only that page's CSS, JS, state assets, and page-owned overlays. It must not emit a root `index.html` as the multi-page runtime.
   - A transition whose `to` is the current screen stays inside that screen HTML; a transition whose `to` is another screen navigates to that screen's generated HTML. A page-owned overlay never becomes a separate HTML document. Verify the generated `pages` and `initial_html` records in `flow-build.json`.
   - In a flow build, each screen's default state's `manifest.json` is the authoritative canvas for that HTML document. Its real PSD width and height determine that page's stage aspect ratio and coordinate conversion; do not apply one global `project.designHeight` to every screen. Different independent screens may use different PSD dimensions.
   - A page-owned overlay is composited into its screen and must use exactly the same canvas width, height, and coordinate alignment as that screen's default PSD. `build_flow.py` must fail when an overlay/state manifest has different dimensions; do not resize or silently reinterpret an overlay to make it fit.
   - `scripts/validate_output.py` supports both direct exports and flow builds. In flow mode it must inspect every generated page HTML/CSS/JS, every state manifest (including states with zero exported assets), every referenced layer asset, and all fatal/effect-rendering errors. A successful validation is required before reporting the build complete.
...
6. Read `manifest.json` or `flow-build.json` and improve the generated H5:
   ...
   - Replace simple text-layer PNGs with HTML text only when the font, weight, line height, color, letter spacing, and paragraph layout can be identified reliably. Keep the raster export as a fallback during visual comparison. Preserve PSD text bounds as the text box for paragraph layers: extract explicit leading or auto-leading, paragraph alignment, indentation, and spacing metadata; paragraph text must preserve explicit line breaks while wrapping inside its PSD width, and must not overflow horizontally. Single-line text remains `white-space: nowrap` with `overflow: visible`. Verify semantic paragraphs against the PSD preview before delivery, especially when their text box extends beyond the canvas.
...
7. Validate the generated output:
   ...
   - Use browser automation to exercise every generated interaction and check console/page errors. For each transition, verify that its source state's resolved `elements[]` contains the trigger and that clicking the hotspot changes to the declared target state.
