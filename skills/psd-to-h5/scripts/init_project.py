...
def main() -> None:
    ...
    flow = {
        ...
        "_guide": {
            ...
            "screens[]": {"作用": "独立页面列表，每个 screens 项代表一个页面，并生成一个独立 HTML 文档；页面内弹窗仍归属于该 HTML。", "示例": "我的、订单列表、商品详情分别配置为不同 screen。"},
            "screens[].id": {"作用": "页面唯一 ID，页面跳转时使用；未填写 title 时也作为 HTML 文件名来源。", "示例": "我的"},
            "screens[].title": {"作用": "可选的页面显示标题和 HTML 文件名来源；未填写时使用 id。程序会清理非法字符并为重名页面追加序号。", "示例": "我的页面"},
            ...
            "transitions[]": {"作用": "描述页面跳转或页面内 overlay 打开/关闭；to 等于当前 screen 时留在当前 HTML，to 为其他 screen 时跳转到对应页面 HTML。", "示例": "我的点击订单入口跳转到订单列表。"},
        ...
        "_instructions": [
            ...
            "每个 screens 项都会生成一个按页面标题命名的 HTML 文件及同名 .assets 资源目录；不要把独立页面实现为同一 HTML 内的多个隐藏容器，也不会默认生成多页面总入口 index.html。",
