    ...
    if manifest_path.is_file():
        ...
        for state_key, state in flow.get("states", {}).items():
            for layer in state.get("layers", []):
                if not layer.get("asset") or not (output / layer["asset"]).is_file():
                    failures.append(f"[{state_key}] missing asset for layer {layer.get('name')!r}: {layer.get('asset')}")
        seen = set()
        for state_key, state in flow.get("states", {}).items():
            layer = next((l for l in state.get("layers", []) if l.get("asset")), None)
            if not layer:
                continue
            # flow-build records assets as <asset_dir>/<state_id>/assets/<file>,
            # so the state directory is two levels above the asset file.
            state_dir = (output / layer["asset"]).parent.parent
            ...
            if not state_manifest.is_file():
                ...
                continue
            checked += 1
            failures.extend(check_manifest(state_dir, json.loads(state_manifest.read_text(encoding="utf-8")), state_key))
        if not checked:
