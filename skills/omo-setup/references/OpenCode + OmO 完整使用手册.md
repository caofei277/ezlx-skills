...
**模型**：GLM-4.7（1倍消耗，默认）/ 可手动 `/models` 切换到 GLM-5-Turbo（⚠️高峰3倍/非高峰2倍，长链路规划优化）
...
### 模式三：Prometheus 精确规划模式（默认 GLM-4.7，1倍消耗）
...
步骤 2：回答 Prometheus 的采访问题（使用 GLM-4.7，1倍消耗；复杂任务可 `/models` 切换到 GLM-5-Turbo）
...
效果：切换到 Prometheus（GLM-4.7，1倍消耗；复杂任务可 `/models` 切换到 GLM-5-Turbo），重新规划思路。
